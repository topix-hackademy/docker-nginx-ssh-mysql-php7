...
# PHP Requirements
RUN apt-get -y install php-xml php-mbstring php-bcmath php-zip php-pdo-mysql php-curl php-gd php-intl php-pear php-imagick php-imap php-mcrypt php-memcache php-apcu php-pspell php-recode php-tidy php-xmlrpc php-ldap

# mysql config
RUN sed -i -e"s/^bind-address\s*=\s*127.0.0.1/explicit_defaults_for_timestamp = true\nbind-address = 0.0.0.0/" /etc/mysql/mysql.conf.d/mysqld.cnf

# nginx config
RUN sed -i -e"s/user\s*www-data;/user YOURUSER www-data;/" /etc/nginx/nginx.conf
...
RUN sed -i -e "s/user\s*=\s*www-data/user = YOURUSER/g" /etc/php/7.0/fpm/pool.d/www.conf
...
# Add system user for YOURUSER
RUN useradd -m -d /home/YOURUSER -p $(openssl passwd -1 'YOURUSERPSW') -G root -s /bin/bash YOURUSER \
    && usermod -a -G www-data YOURUSER \
    && usermod -a -G sudo YOURUSER \
    && ln -s /usr/share/nginx/www /home/YOURUSER/www

RUN chown -R YOURUSER:www-data /usr/share/nginx/www \
